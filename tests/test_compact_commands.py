...
monitor = importlib.import_module("xbox_monitor")
...
def test_display_prefix_is_short(monkeypatch, host, executable, method):
    ...
    monkeypatch.setattr(sys, "argv", ["/private/long/tools/xbox_monitor.py" if method == "manual" else "/private/long/bin/xbox_monitor"])
    monkeypatch.setattr(monitor, "__file__", "/private/long/tools/xbox_monitor.py")
    ...
    expected = executable + " xbox_monitor.py --setup" if method == "manual" else "xbox_monitor --setup"
...
def test_help_does_not_display_runtime_paths():
    ...
    assert "xbox_monitor.py --setup" in result.stdout
