...
REPOSITORY_URL = "https://github.com/misiektoja/xbox_monitor"
...
class TestGovernanceDocuments:
    ...
    def test_the_requirements_file_matches_the_packaged_dependencies(self):
        ...
        listed = {match.group(0).casefold().replace("_", "-") for line in read_asset("requirements.txt").splitlines() if line.strip() and not line.lstrip().startswith("#") if (match := re.match(r"[A-Za-z0-9_.-]+", line))}
