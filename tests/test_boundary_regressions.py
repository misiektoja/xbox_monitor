...
def reject_delivery(monkeypatch, token):
    ...
    # Captures a prepared HTTPX request at the transport boundary
    def httpx_reply(self, request):
        seen.append(request)
        monkeypatch.setattr(monitor, "WEBHOOK_HEADERS", {})
        return httpx.Response(401, text="Denied " + token, request=request)
...
@pytest.mark.parametrize("url", ["https://ntfy.example.test:bad/topic", "https://ntfy.example.test:99999/topic", "https://ntfy.example.test/topic\x00"])
...
def run_with_invalid_runtime_setting(tmp_path, monkeypatch, arguments):
    config = tmp_path / "monitor.conf"
    config.write_text('XBOX_CHECK_INTERVAL = "abc"\n', encoding="utf-8")
...
def test_recovery_commands_run_with_an_invalid_runtime_setting(tmp_path, monkeypatch, capsys, command):
    ...
    output = capsys.readouterr().out
    assert "Warning: Invalid settings: XBOX_CHECK_INTERVAL must be a number greater than zero" in output
    # Whatever stops the command next, it is no longer the setting the command does not use
    assert "Error: Invalid settings" not in output
    assert monitor.XBOX_CHECK_INTERVAL == monitor.BUILT_IN_RUNTIME_SETTINGS["XBOX_CHECK_INTERVAL"]
...
def test_a_monitoring_run_stops_on_an_invalid_runtime_setting(tmp_path, monkeypatch, capsys):
    run_with_invalid_runtime_setting(tmp_path, monkeypatch, ["ExampleTag"])
    assert "Error: Invalid settings: XBOX_CHECK_INTERVAL must be a number greater than zero" in capsys.readouterr().out
