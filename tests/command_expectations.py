...
def runtime_command(command, prefix=None):
    module = importlib.import_module("xbox_monitor")
    assert module.__file__ is not None
    manual = [sys.executable, str(Path(module.__file__).resolve())]
    packaged = [sys.executable, "-m", "xbox_monitor"]
    ...
    command = re.sub(r"(?<![\w/])python(?:3(?:\.\d+)?)? xbox_monitor\.py", lambda match: manual_prefix, command)
    return re.sub(r"(?<![\w/])xbox_monitor(?= |$)", lambda match: packaged_prefix, command)
